-- 修正账务自检不变式 I4 的 SQL
--
-- 原实现使用 "(a, b) IS DISTINCT FROM (SELECT x, y ...)" 形式，
-- PostgreSQL 不允许 IS DISTINCT FROM 的右侧是多列子查询（报 42601）。
-- 改为 LEFT JOIN LATERAL 先聚合成一行再逐列比较。
--
-- 本迁移整体重放 01-triggers.sql，同时保证此前对函数/触发器的改动一并生效。
-- =============================================================================
--  数据库层强制约束（最后一道防线）
--  ============================================================================
--  为什么要在数据库层再做一遍？
