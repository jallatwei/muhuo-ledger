...
/**
 * 是不是「出租车卷式 / 通用机打发票」这类**不是扣税凭证**的出租车票。
 *
 * ★ 为什么必须单独判它，而不是让它走上面的旅客运输倒算：
 *   出租车属于公路运输，13号公告给"列明旅客身份信息的公路、水路等其他客票"
 *   规定的算法是 ÷(1+3%)×3%。如果有人（模型、或将来某段逻辑）看到"出租车"
 *   就按 3% 倒算，就会给一张**本来不能抵扣**的凭证凭空算出进项税 ——
 *   那是实打实的少缴税风险，不是小数目：每 100 元车费多抵 2.91 元。
 *
 *   按 13号公告（二）2 与上海税务口径，出租车票不能抵扣的原因是
 *   **未注明旅客身份信息、且不是增值税扣税凭证**，而不是"免税"。
 *   两者税额都为 0、全额进费用，但口径写错迟早误导人。
 *
 * ★ 但要放过网约车/出租车公司开的**增值税电子普通发票** ——
 *   那是合规扣税凭证，票面列明税额，凭票直抵，不能一起清零。
 *   所以类别里出现"增值税"或"电子发票"就不算这类。
 */
export function isNonDeductibleTaxiVoucher(category: string | null | undefined): boolean {
  const s = (category ?? '').trim();
  if (!s) return false;
  // 增值税发票（含电子发票）是合规扣税凭证，不在此列
  if (/增值税|电子发票/.test(s)) return false;
  return /出租车|的士|卷式|通用机打发票|机打发票/.test(s);
}
